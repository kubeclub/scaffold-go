...
import (
	...
	"flag"
	"fmt"
	dlog "exexm.com/golang_common/log"
	"io/ioutil"
	"log"
	"math/rand"
	"net"
...
func Init(configPath string) error {
	return InitModule(configPath, []string{"base", "mysql", "redis","es"})
}
...
func InitModule(configPath string, modules []string) error {
	...
	// 加载es配置
	if InArrayString("es", modules) {
		if _,err := InitEs(GetConfPath("es")); err != nil {
			fmt.Printf("[ERROR] %s%s\n", time.Now().Format(TimeFormat), " InitEsConf:"+err.Error())
		}
	}
